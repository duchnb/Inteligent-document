...
// Show demo mode banner if no access token
const urlParams = new URLSearchParams(window.location.search);
if (!urlParams.get('access')) {
    const banner = document.createElement('div');
    banner.className = 'demo-banner';
    banner.innerHTML = '📌 Demo Mode: Try search with pre-indexed documents. Full access via CV link.';
...
async function doUpload() {
    ...
    if (f.size > 5 * 1024 * 1024) {
        setStatus('File too large. Max 5MB for demo.', false);
        return;
    }
    ...
    try {
        ...
        if (!upMetaRes) {
            setStatus('Upload URL error (network)', false);
            return;
        }

        let upMeta = {};
        try {
            upMeta = await upMetaRes.json();
        } catch {
            upMeta = {};
        }
